...
public class HighwayGenerator : MonoBehaviour {
    ...
    void BranchHighway() {
        branchDistance++;
        if ( branchDistance < settings.minimalBranchDistance) {
            int spawnNumber = Random.Range(0, 100 + 1);
            if (spawnNumber < settings.branchProbability) {
                GameObject branch = Instantiate(BranchPrfab,null);
                branch.GetComponent<HighwayGenerator>().InitBranch(this.transform.eulerAngles, this.transform.position);
                branchDistance = 0;
            }
        }
    }
